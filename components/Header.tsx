...
const Header: React.FC<HeaderProps> = ({ settings }) => {
  ...
  return (
    <header className="w-full z-50 sticky top-0 shadow-md">
      <div className="ticker-container py-2 border-b text-xs md:text-[11px] font-bold text-gray-500 uppercase tracking-widest bg-gray-50">
            ...
            <Link to="/" className="flex items-center group">
              {settings.headerLogo ? (
                <img 
                  ...
                  alt="Hitech Hardware Logo" 
                  className="h-8 md:h-10 lg:h-11 w-auto object-contain transition-transform group-hover:scale-105" 
                />
              ) : (
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xl md:text-xl font-bold leading-none tracking-tight">HITECH</span>
          ...
          <div className="px-4 pt-2 pb-6 space-y-1">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="block px-3 py-4 text-base font-bold border-b border-gray-100 last:border-0"
            ...
            <div className="px-3 pt-4">
              <Link
                to="/contact"
                className="block w-full text-center bg-primary text-white py-3.5 rounded-xl font-bold text-base shadow-lg active:scale-95 transition-all"
                onClick={() => setIsOpen(false)}
              >
                Contact Us
              </Link>
