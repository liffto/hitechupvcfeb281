...
import { Save, Plus, Trash2, Phone, Image as ImageIcon, Briefcase, Star, Layout, ShieldAlert, Key, Globe, Mail, Facebook, MapPin, Award } from 'lucide-react';
...
const AdminSettings: React.FC<AdminSettingsProps> = ({ settings, setSettings, adminPassword, setAdminPassword }) => {
  ...
  const addCertificate = () => {
    if (newCertificate.trim()) {
      setFormData({ ...formData, certificates: [...(formData.certificates || []), newCertificate] });
      setNewCertificate('');
    }
  };
  ...
  return (
    <div className="flex bg-gray-50 min-h-screen">
      ...
      <div className="flex-grow lg:ml-64 p-4 md:p-8 pt-20 lg:pt-8 overflow-x-hidden">
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 mb-8">
          <h2 className="text-xl font-black text-black mb-6 flex items-center gap-2">
            <Award size={22} className="text-primary" /> Company Certificates
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {(formData.certificates || []).map((url, idx) => (
              <div key={idx} className="relative aspect-[3/4] bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden group">
                <img src={url} className="w-full h-full object-contain" alt="" />
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <button 
                    onClick={() => removeCertificate(idx)}
                    className="p-4 bg-white text-red-600 rounded-full shadow-xl hover:scale-110 transition-transform"
                  >
                    <Trash2 size={24} />
                  </button>
                </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Instagram URL</label>
              <input type="text" className="w-full p-4 border-2 border-gray-100 bg-gray-50 rounded-2xl outline-none font-bold text-gray-800 text-base" value={formData.instagram} onChange={e => setFormData({...formData, instagram: e.target.value})} placeholder="https://instagram.com/yourpage" />
            </div>
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Hitech Head (Admin Phone)</label>
            ...
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Tirupur (Ho) Phone</label>
            ...
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Coimbatore Phone</label>
            ...
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Coimbatore Landline</label>
              <input type="text" className="w-full p-4 border-2 border-gray-100 bg-gray-50 rounded-2xl outline-none font-bold text-gray-800 text-base" value={formData.coimbatoreLandline} onChange={e => setFormData({...formData, coimbatoreLandline: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Madurai Phone</label>
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 mb-8">
          <h2 className="text-xl font-black text-black mb-6 flex items-center gap-2">
            <MapPin size={22} className="text-primary" /> Office Locations
          </h2>
          <div className="grid grid-cols-1 gap-6">
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Head Office Address (Tirupur)</label>
              <textarea rows={2} className="w-full p-4 border-2 border-gray-100 bg-gray-50 rounded-2xl outline-none font-bold text-gray-800 text-base" value={formData.headOfficeAddress} onChange={e => setFormData({...formData, headOfficeAddress: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Coimbatore Office Address</label>
              <textarea rows={2} className="w-full p-4 border-2 border-gray-100 bg-gray-50 rounded-2xl outline-none font-bold text-gray-800 text-base" value={formData.coimbatoreAddress} onChange={e => setFormData({...formData, coimbatoreAddress: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm font-black text-gray-600 uppercase tracking-wider mb-2">Madurai Office Address</label>
              <textarea rows={2} className="w-full p-4 border-2 border-gray-100 bg-gray-50 rounded-2xl outline-none font-bold text-gray-800 text-base" value={formData.maduraiAddress} onChange={e => setFormData({...formData, maduraiAddress: e.target.value})} />
            </div>
          </div>
        </div>
