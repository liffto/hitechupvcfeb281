  ...
  return (
    <Layout settings={settings}>
      <div className="bg-primary py-12 md:py-16 text-center text-white">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Image Gallery</h1>
        ...
        {gallery.length === 0 ? (
          ...
          <div className="columns-1 sm:columns-2 md:columns-3 gap-6 space-y-6">
            {gallery.map((img) => (
              <div 
                key={img.id} 
                className="relative break-inside-avoid group rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-50 bg-gray-100"
              >
                <img 
                  src={img.url} 
      ...
      <div className="bg-[#F4F9F1] py-16 text-center">
         <div className="max-w-2xl mx-auto px-4">
            <h2 className="text-xl md:text-2xl font-bold text-black mb-4 italic">"Quality Hardware, Quality Life."</h2>
